...
2. **Добавить в проект следующие зависимости:**
    - [x] `go get -u github.com/urfave/cli/v2`
    - [x] `go get -u github.com/doug-martin/goqu/v9`
    - [x] `go get -u github.com/gofiber/fiber/v2`
...
далее добавляем в корень проекта файл `.golangci.yml` со следующим содержимым
...
далее из корня проекта выполняем команду

```shell
golangci-lint run
```

тут покажутся ошибки, их нужно будет исправлять :)

каждую ошибку можно гуглить отдельно по названию, например, `goimports`

или считерить и запустить 

```shell
golangci-lint run --fix
```

но он не все ошибки фиксит